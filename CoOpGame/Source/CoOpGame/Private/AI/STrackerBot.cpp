...
void ASTrackerBot::SelfDestruct()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		float ActualDaamage = ExplosionDamage + (ExplosionDamage * PowerLevel);

		// Apply Damage!
		UGameplayStatics::ApplyRadialDamage(this, ActualDaamage, GetActorLocation(), ExplosionRadius, nullptr, IgnoredActors, this, GetInstigatorController(), true);
...
void ASTrackerBot::Tick(float DeltaTime)
{
	...
	if (Role == ROLE_Authority && !bExploded)
	{
		...
		else
		{
			...
			if (DebugTrackerBotDrawing)
			{
				DrawDebugDirectionalArrow(GetWorld(), GetActorLocation(), GetActorLocation() + ForceDirection, 32, FColor::Yellow, false, 0.f, 0, 1.0f);
			}
		}
